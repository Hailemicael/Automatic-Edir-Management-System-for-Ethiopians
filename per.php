...
if(isset($_POST['submit'])){
	$r=$_POST['reason'];
	$dd=date("d/m/Y");
	$d=mysql_query("insert into permission values('$_SESSION[user]','$r','$dd','')");
	if($d){
		echo "<b style='color:green'> Successfully sent the request permission</b>";
	}else{
		echo "<b style='color:red'> You are already sent the request</b>";
	}
...
<input type="submit" name="submit" value="Send Request"/> &nbsp;&nbsp;&nbsp;<input type="reset" value="clear"/>
 ...
 Print "<tr>";
 print "<th colspan='7'><span  style='color: #44a2ee;' /><u> Your Permissions   </u></span></th>";
print "</tr>";
...
Print "<th>Replay From Manager </th>";
 ...
 Print "</tr>"; 
 $data = mysql_query("SELECT * FROM permission WHERE id='$_SESSION[user]' ") or die(mysql_error());
